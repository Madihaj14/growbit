...
export const HabitProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addHabit = (habitData: Omit<Habit, 'id' | 'streak' | 'bestStreak' | 'completedDates' | 'createdAt' | 'completedToday' | 'xp'>) => {
    const difficulty = habitData.difficulty || 'medium';
    // Calculate base XP based on difficulty
    let baseXp = 0;
    switch (difficulty) {
      case 'easy': baseXp = 5; break;
      case 'medium': baseXp = 10; break;
  ...
  const completeHabit = (id: string) => {
    ...
    setHabits((prevHabits) => 
      prevHabits.map((habit) => {
        ...
        // Calculate XP - base plus streak bonus and difficulty bonus
        let baseXp = 10;
        if (habit.difficulty === 'easy') baseXp = 5;
        if (habit.difficulty === 'hard') baseXp = 20;
        
        const streakBonus = Math.min(newStreak * 2, 50); // Cap streak bonus at 50
        const newXp = habit.xp + baseXp + streakBonus;
  ...
  const uncompleteHabit = (id: string) => {
    ...
    setHabits((prevHabits) => 
      prevHabits.map((habit) => {
        ...
        // Reduce XP - base + streak bonus based on difficulty
        let baseXp = 10;
        if (habit.difficulty === 'easy') baseXp = 5;
        if (habit.difficulty === 'hard') baseXp = 20;
        
        const streakBonus = Math.min(habit.streak * 2, 50);
        const xpReduction = baseXp + streakBonus;
